"""
CUSTOM-EMBEDDING 嵌入模型，基于通义千问的文本嵌入模型
...
def load_default_config() -> 'CustomEmbeddingConfig':
    """加载默认配置
    
    从项目根目录的 .env 文件中加载配置参数。
    必需的环境变量：
    - CUSTOM_EMBEDDING_API_KEY: API密钥
    - CUSTOM_EMBEDDING_BASE_URL: API基础URL
    - CUSTOM_EMBEDDING_MODEL_NAME: 模型名称
    ...
    base_url = os.getenv('CUSTOM_EMBEDDING_BASE_URL')
    model_name = os.getenv('CUSTOM_EMBEDDING_MODEL_NAME')
    ...
    return CustomEmbeddingConfig(
        model_name=model_name,
        api_url=base_url,
        api_key=api_key,
        dimension=1024,  # CUSTOM-EMBEDDING 默认维度
...
class CustomEmbedding(DenseEmbeddingModel):
    """CUSTOM-EMBEDDING 嵌入模型实现，支持本地模型和 API 调用"""
    
    def __init__(self, config: Optional[CustomEmbeddingConfig] = None):
        """初始化模型
